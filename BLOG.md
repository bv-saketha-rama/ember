# Ember — tend the things you're building, and watch them burn 🔥

> This is a submission for the [Weekend Challenge: Passion Edition](https://dev.to/challenges/weekend-2026-07-09).

The canonical Dev.to-ready copy lives in [`dev.to/blog.md`](dev.to/blog.md).

## Quick links

- [Live app](https://ember-two-sigma.vercel.app)
- [Source code](https://github.com/bv-saketha-rama/ember)
- [Dev.to-ready assets](dev.to/assets/)
